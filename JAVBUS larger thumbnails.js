...
(function () {
    ...
    let showAlert = (msg, close) => {
        ...
        if (close) {
            let $close = $(
                `<div style="display: inline-block;padding: 0 0 0 10px;color:gray;cursor: pointer;">X</div>`,
            );
            $alert.append($close);
    ...
    class SettingMenu {
        ...
        constructor() {
            ...
            let $circle = $(
                `<div style="position: ${currentWeb == "javlibrary" ? "absolute" : "fixed"};z-index: 1030;left:0;top:${currentWeb == "javlibrary" ? "36px" : "0px"};"><div style="width: 40px;height: 40px;background-color: rgb(208 176 176 / 90%);border-radius: 20px;"></div></div>`,
            );
            $circle.append($menu);
        ...
        creatCheckbox(fName, name, disabled) {
            ...
            let me = this;
            let $checkbox = $(
                `<div class="switch-div"><input ${disabled ? 'disabled="disabled"' : ""} type="checkbox" id="${fName}_checkbox" /><label for="${fName}_checkbox" >${name}</label></div>`,
            );
    ...
    //获取javdb的演员、预览图、磁力信息
    GM_addStyle(`#modal-div .pop-up-tag {display: grid;  grid-template-columns: repeat(2, 1fr);grid-auto-rows: minmax(100px, auto);grid-template-areas:"a a""b b""c d";}
    .pop-up-tag .panel-block{grid-area: a;}.pop-up-tag .columns:nth-child(even){grid-area: b;}.pop-up-tag .columns:nth-child(odd){grid-area: c;max-height: 472px;overflow-x: hidden;}.pop-up-tag nav{grid-area: d;}`);
    async function getMagnet4JavDB(href, tagName, itemID, avid) {
        ...
        if (Status.get("avInfo")) {
            ...
            $preview_images
                .find(".preview-video-container")
                .attr("href", `#preview-video-${itemID}`);
            $preview_images
                .find("#preview-video")
                .attr("id", `preview-video-${itemID}`);
        ...
        let magnetTable = $doc.find(`div.columns[data-controller="movie-tab"]`); //磁力链接
        magnetTable.find("div.top-meta").remove(); // 移除广告
        let movie_panel_info = $doc.find(".movie-panel-info");
        info.append(magnetTable).append(movie_panel_info);
        return info;
    }
    // javbus：获取演员磁力信息
    async function getMagnet4JavBus(href, tagName) {
        ...
        let table_tag = $(
            `<table class="table pop-up-tag" name="${tagName}" style="background-color:#FFFFFF;" ></table>`,
        );
    ...
    function creatCopybutton(text) {
        let $copyButton = $(
            `<td><button class="center-block">${lang.copyButton}</button></td>`,
        );
    ...
    async function avInfofetch(href, tagName) {
        let doc = await fetch(href, { method: "GET", mode: "no-cors" }).then(
        ...
        if (Status.get("avInfo")) {
            ...
            if (avatar_waterfall.length > 0) {
                ...
                avatar_waterfall.find("a.avatar-box span").each((i, el) => {
                    let $copySvg = $(
                        `<div style="width:24px;height:24px;display: flex;align-items: center;justify-content: center;">${copy_Svg}</div>`,
                    );
                ...
                avatar_waterfall
                    .find("a.avatar-box")
                    .attr("target", "_blank")
                    .removeClass("avatar-box")
                    .addClass("avatar-box-zdy");
            ...
            if (sample_waterfall.length > 0) {
                ...
                sample_waterfall
                    .find(".sample-box")
                    .removeClass("sample-box")
                    .addClass("sample-box-zdy");
   ...
   */
    ...
    class ScrollerPlugin {
        ...
        domWatch() {
            let me = this;
            if (
                me.$page.get(0).getBoundingClientRect().top - $(window).height() <
                300 &&
                !me.locked &&
                me.canLoad
            ) {
        ...
        async loadNextPage(url) {
            ...
            if (
                currentWeb != "javdb" &&
                location.pathname.includes("/star/") &&
                elems
            ) {
                elems = elems.slice(1);
            }
    ...
    const addStyle = () => {
        ...
        #myModal #modal-div{position:relative;width:70%;margin:0 auto;background-color:rgb(6 6 6 / 50%);border-radius:8px;animation:fadeInDown .5s}
